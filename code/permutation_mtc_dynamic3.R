...
qtl_files <- paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-", seq(1, chunks_tot), ".permuted3.tsv")
results <- map(qtl_files, read_tsv) %>% 
  bind_rows

day_files <- paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-", seq(1, chunks_tot), ".day.permuted3.tsv")
days.permuted <- map(day_files, read_tsv) %>% 
  bind_rows

double.up <- duplicated(paste(results$gene, results$snp))
results <- filter(results, !double.up)
days.permuted <- filter(days.permuted, !double.up)

pdist.days <- apply(days.permuted, 1, function(x){kendall.tau(x, colnames(days.permuted))})
results$tau <- pdist.days
...
saveRDS(g, paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs.ashr.g.permuted3.rds"))
...
write_tsv(results, paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, ".mtc.permuted3.tsv"))
write_tsv(top.hits, paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, ".tophits.permuted3.tsv"))
