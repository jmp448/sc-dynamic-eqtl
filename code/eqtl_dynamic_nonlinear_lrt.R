...
# get a snp list
snps <- tibble("snp"=colnames(genotypes)[-c(1)]) %>%
...
formula.str <- paste0("exp~1+", samppc.str, ctprop.str, "(", clpc.str, "g)*day+(", clpc.str, "g)*day.sq")

eqtl.call <- function(i, fname.coef) {
  snp.i = all.tests$snp[i]
  gene.i = all.tests$gene[i]
  design = left_join(genotypes[,c(1,(filter(snps, snp==snp.i)$index)+1)], days, by="sample") %>% 
    rename(g=starts_with("chr")) %>%
    left_join(cvrts, by="sample") %>%
    mutate(exp=expr[[gene.i]])
  model.full = lm(formula(formula.str), design)
  model.null=lm(formula(paste0(formula.str, "-g:day-g:day.sq")), design)
  
  lrt <- anova(model.full, model.null)[[6]][2]
  
  coef.out = tibble("gene"=gene.i, "snp"=snp.i, 
                    "beta.gxt2"=coefficients(model.full)["g:day.sq"], 
                    "se.gxt2"=coefficients(summary(model.full))["g:day.sq",2],
                    "beta.gxt"=coefficients(model.full)["g:day"],
                    "se.gxt"=coefficients(summary(model.full))["g:day",2],
                    "p"=lrt) %>%
...
coef.file <- paste0("../results/eqtl_dynamic/nonlinear_dQTL/", dataset, "/", agg, "/lrt/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-", chunk_i, ".tsv")

if (file.exists(coef.file)) {
  file.remove(coef.file)
}

coef.header <- c("gene", "snp", "beta.gxt2", "se.gxt2", "beta.gxt", "se.gxt2", "p") %>% 
  purrr::map_dfc(setNames, object=list(logical())) 
write_tsv(coef.header, coef.file, col_names=T)

system.time(lapply(1:nrow(all.tests), eqtl.call, coef.file))
