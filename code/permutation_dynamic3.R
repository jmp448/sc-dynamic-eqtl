...
snps <- split(all.tests$snp, cut(seq_along(all.tests$snp), chunks_tot, labels = FALSE))[[chunk_i]]
all.tests <- filter(all.tests, snp %in% snps)
snp_num <- tibble("snp"=unique(all.tests$snp)) %>%
  rowid_to_column("snp_num")
all.tests <- left_join(all.tests, snp_num, by="snp")
...
if (agg == "day") {
  ...
  days <- read_tsv(paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/bin_medians.tsv")) %>%
...
if (cell.type.reg) {
  if (dataset == "bulk") {
    ...
    ct.props <- read_tsv("../results/cibersort/bulk.inferred.tsv") %>%
      select(!c(`P-value`, Correlation, RMSE)) %>% rename(sample=Mixture) %>%
      arrange(sample) %>% select(!UNK) %>% fitler(sample %in% expr$sample)
...
formula.str <- paste0("~1+", samppc.str, ctprop.str, "(", clpc.str, "g)*day.permute")


permuter <- tibble("bin"=unique(days$bin)) %>% arrange(bin)
bins <- permuter$bin
nbins <- length(bins)
eqtl.call <- function(i, fname.coef, fname.dperm, fname.mc) {
  if (i %% 1000 == 0) {
    print(paste0("i >= ", i))
  }
  snp = all.tests$snp[[i]]
  gene = all.tests$gene[[i]]
  exp.snp = t(select(expr, all_of(gene)))
  permuter$bin.permuted <- sample(bins, nbins)
  design = left_join(genotypes[,c(1,all.tests$snp_num[i]+1)], days, by="sample") %>% 
    rename(g=starts_with("chr")) %>%
    left_join(permuter, by="bin") %>%
    mutate(day.permuted=day[match(paste(ind, bin.permuted, sep="_"), sample)]) %>%
  ...
  if (anyNA(coefficients(model)[,"g:day.permute"])) {
    write_tsv(tibble(gene=gene, snp=snp), fname.mc, append=T)
    return()
  }
  coef.out = tibble("gene"=gene, "snp"=snp, "beta.g"=as.numeric(model$coefficients[,"g"]),
                    "beta.gxt"=as.numeric(model$coefficients[,"g:day.permute"]),
                    "beta.t"=as.numeric(model$coefficients[,"day.permute"]),
                    "stdev.unscaled.g"=as.numeric(model$stdev.unscaled[,"g"]), 
                    "stdev.unscaled.gxt"=as.numeric(model$stdev.unscaled[,"g:day.permute"]),
                    "stdev.unscaled.t"=as.numeric(model$stdev.unscaled[,"day.permute"]),
                    "sigma"=as.numeric(model$sigma), "df.residual"=as.numeric(model$df.residual)) %>%
    write_tsv(fname.coef, append=T)
  dperm.out <- write(permuter$bin.permuted, fname.dperm, ncolumns=nbins, append=T, sep="\t")
}
...
coef.file <- paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-", chunk_i, ".permuted3.tsv")
dperm.file <- paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-", chunk_i, ".day.permuted3.tsv")
mc.file <- paste0("../results/eqtl_dynamic/linear_dQTL/", dataset, "/", agg, "/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-", chunk_i, ".multicollinear_tests.permuted3.tsv")

if (file.exists(coef.file)) {
  file.remove(coef.file)
}
if (file.exists(dperm.file)) {
  file.remove(dperm.file)
}
...
dperm.header <- bins %>% purrr::map_dfc(setNames, object=list(logical()))
write_tsv(dperm.header, dperm.file, col_names=T)
mc.header <- tibble("gene"=character(), "snp"=character())
write_tsv(mc.header, mc.file, col_names=T)

system.time(lapply(1:nrow(all.tests), eqtl.call, coef.file, dperm.file, mc.file))