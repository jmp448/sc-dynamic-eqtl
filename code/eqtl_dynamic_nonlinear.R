...
eqtl.call <- function(i, fname.coef) {
  ...
  coef.out = tibble("gene"=snp.genes, "snp"=snps[i],
                    "beta.gxt2"=as.numeric(model$coefficients[,"g:day.sq"]),
                    "stdev.unscaled.gxt2"=as.numeric(model$stdev.unscaled[,"g:day.sq"]),
...
coef.file <- paste0("../results/eqtl_dynamic/nonlinear_dQTL/", dataset, "/", agg, "/t_test/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-", chunk_i, ".tsv")

if (file.exists(coef.file)) {
  file.remove(coef.file)
}

coef.header <- c("gene", "snp", "beta.gxt2", "stdev.unscaled.gxt2",
                  "sigma", "df.residual") %>% 
