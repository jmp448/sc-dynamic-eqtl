...
hits <- vroom("../results/eqtl_dynamic/nonlinear_dQTL/pseudobulk-cm/bin16/t_test/50k-5clpcs-0pcs-notypes.tophits.tsv") %>%
  filter(qval.unadj<=0.05) %>%
  unite(gv, gene, snp, sep="--") %>%
  select(gv)
all.tests <- unite(all.tests, gv, gene, snp, sep="--", remove=F) %>%
  filter(gv %in% hits$gv) %>%
  select(-gv)
...
formula.str <- paste0("~1+", samppc.str, ctprop.str, "(", clpc.str, "g)*day+(", clpc.str, "g)*day.sq")
...
eqtl.call <- function(i, fname.coef) {
  ...
  coef.out = as_tibble(coefficients(model)) %>%
    select(`(Intercept)`, g, day, day.sq, `g:day`, `g:day.sq`) %>%
    mutate(gene=snp.genes, snp=snps[i], .before=1) %>%
...
coef.file <- paste0("../results/eqtl_dynamic/nonlinear_dQTL/", dataset, "/", agg, "/t_test/", cis.dist, "-", n.cl.pcs, "clpcs-", n.samp.pcs, "pcs-", cell.type.reg, "-coefficients.tophits.tsv")
if (file.exists(coef.file)) {file.remove(coef.file)}

coef.header <- c("gene", "snp", "intercept", "beta.g", "beta.t", "beta.t2", "beta.gxt", "beta.gxt2") %>% 
  purrr::map_dfc(setNames, object=list(logical())) 
write_tsv(coef.header, coef.file, col_names=T)

system.time(lapply(1:length(snps), eqtl.call, coef.file))
