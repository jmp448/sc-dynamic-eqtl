...
bulk.props <- read_tsv("results/cibersort/bulk.inferred.tsv") %>% 
  ...
  mutate(exp="bulk")
p_clpc1 <- ggplot(bulk.props, aes(x=day, y=frac, fill=type)) +
...
diff.speed <- ct.proportions %>% 
  mutate(early=IPSC+MES) %>%
  mutate(ind=substr(sample, 1, 5)) %>%
  group_by(ind) %>%
  summarize(delay=sum(early), cf=sum(CF), cm=sum(CM)) %>%
...
terminal.cell.type <- ct.proportions %>%
  ...
  mutate(type=factor(type, levels=c("CM", "CF"))) %>%
  drop_na %>%
  gather(!c(ind, type), key="covariate", value="value") %>%
  mutate(covariate=factor(covariate, levels=paste0("PC_", seq(1, 5)))) %>%
  filter(covariate %in% c("PC_1", "PC_2"))

p_clpc2 <- ggplot(terminal.cell.type, aes(x=covariate, fill=type, y=value)) +
  geom_boxplot() + theme_classic(base_size=13) + xlab("Cell Line PC") +
  ylab("PC Score") + labs(fill="Primary Cell Type, Days 10-15")

png("figs/supp/clpc_explainer.png", width=600, height=800)
p_clpc1 | p_clpc2 / guide_area() + plot_layout(guides="collect")
dev.off()
```
![](../figs/supp/clpc_explainer.png)
