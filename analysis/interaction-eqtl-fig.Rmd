...
png('figs/fig4/fig4a.png', width=400, height=600)
plot(fig4a)
dev.off()
```
...
```{r}
cm.hits <- read_tsv("results/eqtl_dynamic/ieQTL/bulk/CM/50k-5clpcs-0pcs-notypes.tophits.tsv") %>% filter(qval.unadj<=0.05) %>% .$gene
cf.hits <- read_tsv("results/eqtl_dynamic/ieQTL/bulk/CF/50k-5clpcs-0pcs-notypes.tophits.tsv") %>% filter(qval.unadj<=0.05) %>% .$gene

venn.diagram(
  x = list(cm.hits, cf.hits),
...
cf.hits <- vroom("results/eqtl_dynamic/ieQTL/bulk/CF/50k-5clpcs-0pcs-regtypes.tophits.tsv") %>% filter(qval.unadj<=0.05) %>% select(gene, snp, bonf.p.unadj) %>% mutate(celltype="CF")
