...
Now, can we get more hits if instead of binning pseudobulk by day, we bin by pseudotime bin?
```{r, warning=FALSE, message=FALSE}
...
Finally, we can look specifically at those dynamic eQTLs that _were_ significant at $lfsr<=0.1$ in both experiments
```{r, warning=FALSE, message=FALSE}
bulk <- read_tsv("results/eqtl_dynamic/linear_dQTL/bulk/day/50k-3clpcs.mtc.tsv") %>%
  mutate(gv=paste(gene, snp, sep="--")) %>%
  mutate(bulk.log10p=-log10(p.adj)) %>%
