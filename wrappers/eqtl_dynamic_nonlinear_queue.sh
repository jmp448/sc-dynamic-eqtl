#!/bin/bash
#SBATCH --time=10:0
#SBATCH --output=logs/eqtl_dynamic_nonlinear_queue.out
#SBATCH --error=logs/eqtl_dynamic_nonlinear_queue.err

# rm -r logs/nl-dynamic-eqtl-calling-lrt/
# mkdir logs/nl-dynamic-eqtl-calling/

declare -a CisDists=("50k")
declare -a NumSampPCs=(0)
declare -a NumCLPCs=(0 1 2 3 4)
  ...
  for cdist in ${CisDists[@]}; do
    for nspc in ${NumSampPCs[@]}; do
      for ncpc in ${NumCLPCs[@]}; do
        sbatch --mem=15G -J nl-lrt --error=logs/nl-dynamic-eqtl-calling/bulk-$cdist-$nspc-$ncpc-F-$c.err --output=logs/nl-dynamic-eqtl-calling/bulk-$cdist-$nspc-$ncpc-F-$c.out eqtl_dynamic_nonlinear.sh "bulk" $cdist $nspc $ncpc "F" "day" $c 10 &
...
#         sbatch --mem=15G -J dyn-$c --error=logs/nl-dynamic-eqtl-calling/pb-cf-d-$cdist-$nspc-$ncpc-F-$c.err --output=logs/dynamic-eqtl-calling/pb-cf-d-$cdist-$nspc-$ncpc-F-$c.out eqtl_dynamic.sh "pseudobulk-cf" $cdist $nspc $ncpc "F" "day" $c 10 &
        for b in ${Bins[@]}; do
          sbatch --mem=15G -J dyn-$c --error=logs/nl-dynamic-eqtl-calling/pb-cm-$b-$cdist-$nspc-$ncpc-F-$b-$c.err --output=logs/nl-dynamic-eqtl-calling/pb-cm-$b-$cdist-$nspc-$ncpc-F-$c.out eqtl_dynamic_nonlinear.sh "pseudobulk-cm" $cdist $nspc $ncpc "F" $b $c 10 &
          sbatch --mem=15G -J dyn-$c --error=logs/nl-dynamic-eqtl-calling/pb-cf-$b-$cdist-$nspc-$ncpc-F-$b-$c.err --output=logs/nl-dynamic-eqtl-calling/pb-cf-$b-$cdist-$nspc-$ncpc-F-$c.out eqtl_dynamic_nonlinear.sh "pseudobulk-cf" $cdist $nspc $ncpc "F" $b $c 10 &
        done
      done
    done
  done
