...
for cdist in ${CisDists[@]}; do
  for nspc in ${NumSampPCs[@]}; do  
    for ncpc in ${NumCLPCs[@]}; do
      ...
      for b in ${Bins[@]}; do
        sbatch --mem=15G -J dyn-mtc --error=logs/dynamic-eqtl-mtc/pb-$b-$cdist-$nspc-$ncpc-F.err --output=logs/dynamic-eqtl-mtc/pb-$b-$cdist-$nspc-$ncpc-F.out eqtl_dynamic_mtc.sh "pseudobulk-cm" $cdist $nspc $ncpc "F" $b 10 &
        sbatch --mem=15G -J dyn-mtc --error=logs/dynamic-eqtl-mtc/pb-$b-$cdist-$nspc-$ncpc-F.err --output=logs/dynamic-eqtl-mtc/pb-$b-$cdist-$nspc-$ncpc-F.out eqtl_dynamic_mtc.sh "pseudobulk-cf" $cdist $nspc $ncpc "F" $b 10 &
      done
    done
  done
done
